...
/**
 * @brief Horizontal slider widget supporting float values.
 * Mirrors the design of AudioVerticalSlider but in horizontal orientation.
 */
class AudioHorizontalSlider : public QWidget
{
    Q_OBJECT
public:
    /**
     * @brief Constructs a horizontal slider with default range [0.0, 100.0].
     * @param parent Parent widget.
     */
    explicit AudioHorizontalSlider(QWidget* parent = nullptr);
     ...
     */
    void setDefaultValue(float defaultValue) { this->m_defaultValue = defaultValue; }

    // --- PUBLIC COLORS (matching AudioVerticalSlider) ---
...
private:
    ...
    float valueFromPosition(int x) const;
    int positionFromValue(float value) const;
    void updateTextSizes();
    int limitHandleX(int x, int handleW, const QRect& groove) const;

    float m_min = 0.0f;
    float m_max = 100.0f;
    float m_value = 50.0f;
    float m_defaultValue = 50.0f;
