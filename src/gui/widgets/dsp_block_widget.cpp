...
DSPBlockWidget::DSPBlockWidget(NoteNagaDSPBlockBase* block, QWidget* parent)
    : QWidget(parent), block_(block), mainLayout_(nullptr), deactivateBtn_(nullptr), deleteBtn_(nullptr)
{
    setStyleSheet("background-color: #32353b; color: #fff; border-radius: 8px;");
    setMinimumWidth(260);
...
void DSPBlockWidget::buildUi()
{
    ...
    topBar_->setObjectName("AdvancedDockTitleBar");
    ...
    deleteBtn_ = create_small_button(":/icons/close.svg", "Delete block",
                                     "deleteBtn", 24, topBar_);
    ...
    for (size_t i = 0; i < params.size(); ++i) {
        ...
        if (desc.type == DSPParamType::Float) {
            ...
            if (ui_type == "dial" || ui_type == DSP_UI_TYPE_DIAL) {
                ...
                connect(dial, &AudioDial::valueChanged, this, [this, i](float val){
                    block_->setParamValue(i, val);
                });
                control = dial;
                ...
                dial->showValue(true);
                dial->setValueDecimals(2);
                connect(dial, &AudioDialCentered::valueChanged, this, [this, i](float val){
                    block_->setParamValue(i, val);
                });
                control = dial;
                leftParamLayout->addWidget(control);
                ...
                connect(slider, &AudioVerticalSlider::valueChanged, this, [this, i](int val){
                    block_->setParamValue(i, float(val)/100.0f);
                });
                control = slider;
            ...
            if (ui_type == DSP_UI_TYPE_SLIDER_VERTICAL) {
                ...
                slider->setValueVisible(true);
                connect(slider, &AudioVerticalSlider::valueChanged, this, [this, i](int val){
                    block_->setParamValue(i, float(val));
                });
                control = slider;
                rightParamLayout->addWidget(control);
            }
            ...
            btn->setStyleSheet(
                "QPushButton { background-color: #3e4a5a; color: #fff; }"
        ...
        if (control) {
            paramWidgets_[i] = { control, ui_type };
        }
    }
...
void DSPBlockWidget::updateActivationButton()
{
    ...
    deactivateBtn_->setIcon(QIcon(block_->isActive() ? ":/icons/active.svg" : ":/icons/inactive.svg"));
...
void DSPBlockWidget::onDeleteClicked()
{
